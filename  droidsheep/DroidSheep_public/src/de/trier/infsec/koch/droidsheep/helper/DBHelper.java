...
public class DBHelper {
	...
	public static void setGeneric(Context c, boolean b) {
		initDB(c);
		Cursor cur = droidsheepDB.rawQuery("SELECT count(id) as count FROM DROIDSHEEP_PREFERENCES where name = 'generic';", new String[]{});
		cur.moveToFirst();
		int count = (int) cur.getLong(cur.getColumnIndex("count"));
		if (count == 0) {
			droidsheepDB.execSQL("INSERT INTO DROIDSHEEP_PREFERENCES (name, value) values ('generic', ?);", new String[]{Boolean.toString(b)});
		} else {
			droidsheepDB.execSQL("UPDATE DROIDSHEEP_PREFERENCES SET value=? WHERE name='generic';", new String[]{Boolean.toString(b)});
		}
		droidsheepDB.close();
	}
